...
\title{%
Gravitational Backreaction from Relative Entropy: A Variational Principle
}
...
We propose a variational principle for gravitational backreaction based on the relative entropy of quantum fields in causal diamonds. By minimizing a ``coherence functional'' that balances the area deficit of the diamond boundary against the relative entropy of the bulk state, we derive the linearized Einstein equations with a source term proportional to the modular energy. Crucially, because the action vanishes for the reference vacuum state, this framework naturally degravitates vacuum energy, offering a mechanism to resolve the cosmological constant problem. We further show that this feedback mechanism leads to non-local entropic corrections to the stress tensor, which we calculate explicitly for a scalar field. These corrections manifest as an effective ``dark source'' that mimics dark matter in galactic rotation curves and dark energy in cosmological expansion. We propose a specific experimental test using atom interferometry to detect the predicted entropic noise. Bose--Einstein condensate free-fall, and gravitational-wave memory---that could constrain or refute these entropic corrections. The resulting picture treats gravity and quantum mechanics as complementary limits of a deeper feedback law governing the alignment of quantum information with spacetime geometry.
\end{abstract}
...
We call the resulting framework \emph{Entropic Feedback}.
...
\emph{Remark (Degravitation).} A key feature of using relative entropy is that the reference state $\sigma$ (the vacuum) contributes zero to the action, since $S(\sigma||\sigma)=0$. This naturally degravitates the vacuum energy density, addressing the cosmological constant problem without fine-tuning.
...
Specifically, we define $\sigma[g]$ as the \emph{local Unruh state} associated with the diamond's causal development. For general regions, $K_\sigma$ is non-local. However, in the limit of small causal diamonds ($L \ll L_{\mathrm{curvature}}$), the geometry is approximately flat, and the modular Hamiltonian approaches the local boost generator~\cite{Casini2011}. We work in this local limit. Operationally, this is the state that maximizes the von Neumann entropy given the boundary data and the modular Hamiltonian $K_\sigma$ generated by the diamond's boost Killing vector.
...
In the 1+1D entropic toy model, the informational stress can be written schematically as
\begin{equation}
    \Tmn
    =
    (\beta-\alpha)\,\Delta\!\langle T_{\mu\nu}\rangle
...
These deviations are constrained by current data but may leave a small window for entropic corrections.
A more quantitative analysis would require integrating the entropic term into a Boltzmann code, as outlined in Appendix~\ref{appendix:boltzmann}.
...
This ``Entropic Feedback'' recovers semiclassical gravity in the incoherent limit but predicts deviations for macroscopic quantum superpositions.
...
\begin{itemize}
    ...
    \item Understanding the renormalization-group flow of the coupling $\kappa(\mu)$. We treat $\kappa$ as an effective coupling at the scale of the experiment, but its UV behavior requires a deeper theory.
    ...
    \item Embedding the framework into a UV-complete theory of quantum gravity, if possible.
    \item Constructing simple but realistic 3+1D models for experimental tests.
    \item The natural degravitation of the vacuum energy due to the property $S(\sigma||\sigma)=0$.
\end{itemize}
...
We sketched how entropic corrections may appear in cosmology and black-hole physics~\cite{Verlinde2011,Padmanabhan2010}, and identified experimental directions where such effects could be probed.
...
\end{equation}
\footnote{Strictly speaking, varying the metric also deforms the causal diamond boundary $\partial \Ddom$. In this work, we adopt a fixed-background approximation where boundary variation terms are assumed to be higher-order in $(L_P/L)^2$. A full treatment of boundary terms is left for a UV-complete analysis.}
...
Standard quantum mechanics assumes a fixed background; this framework suggests the background relaxes to accommodate the coherent wavefunction.
...
In this framework, the action is augmented by the coherence functional:
\begin{equation}
