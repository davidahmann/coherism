...
\title{%
Coherence-Dependent Backreaction in Semiclassical and Analog Gravity: Testable Predictions from Informational Stress
}
...
\begin{abstract}
We derive a testable prediction for analog gravity experiments in Bose-Einstein condensates: coherent phonon injection near a sonic horizon produces density modulations $\delta\rho/\rho_0 \sim 10^{-6}$, while thermal phonon injection of identical energy produces no such effect. This coherent-versus-thermal signature---measurable with current BEC technology---constitutes a qualitative test absent in competing frameworks (Penrose-Di\'{o}si, stochastic gravity). The prediction emerges from an \emph{informational stress tensor} $\Tmn$ derived by varying a coherence functional $\Cfunc[g,\rho;\mu]$ built from relative entropy and generalized entropy. In analog systems, the effective coupling $\kappa_{\mathrm{eff}} \sim 10^{-8}$ is 62 orders of magnitude larger than the gravitational coupling, bringing predictions within experimental reach. We provide: (i) explicit derivations of $\Tmn$ for Rindler and acoustic geometries; (ii) two independent derivations of $\kappa$ (holographic and entropic); (iii) a first-principles derivation of the Lindblad generator from Unruh-DeWitt dynamics; and (iv) an experimental protocol with falsification criteria. The framework reduces to semiclassical gravity when $\rho \approx \sigma[g]$. A null result at $\delta\rho/\rho_0 < 10^{-7}$ for the BEC experiment would falsify the acoustic implementation.
\end{abstract}

\keywords{analog gravity \and Bose-Einstein condensate \and quantum coherence \and semiclassical gravity \and relative entropy \and informational stress}
...
Can analog gravity experiments distinguish quantum coherence from thermal noise? We show that a minimal extension of semiclassical gravity predicts \emph{yes}---and that this prediction is testable with current Bose-Einstein condensate (BEC) technology.

The key result is this: when coherent phonons are injected toward a sonic horizon in a BEC, our framework predicts a density modulation $\delta\rho/\rho_0 \sim 10^{-6}$ near the horizon. When \emph{thermal} phonons of identical energy are injected, no such modulation appears. This coherent-versus-thermal signature is absent in standard semiclassical gravity (where both produce identical $\langle T_{\mu\nu}\rangle$), absent in Penrose-Di\'{o}si models (which depend on superposition size, not coherence), and absent in stochastic gravity (which treats both states identically). The effect is within reach of current experiments at MIT~\cite{Ketterle2002}, JILA~\cite{Cornell1995}, Technion~\cite{Steinhauer2016}, and MPQ~\cite{Bloch2008}.

\subsection{The Physical Question}

Semiclassical gravity uses the expectation value $\langle T_{\mu\nu}\rangle$ as a source in Einstein's equations~\cite{HuVerdaguer2008}. This hybrid description treats two quantum states identically if they have the same stress-energy expectation value---even if one is a pure coherent state and the other is an incoherent thermal mixture. But should gravity truly be blind to this distinction?

Several lines of evidence suggest otherwise. Thermodynamic derivations of Einstein's equations~\cite{Jacobson1995} and entanglement-based formulations in AdS/CFT implicate entropy and information geometry in the structure of spacetime. Penrose~\cite{Penrose1996} and Di\'{o}si~\cite{Diosi1989} proposed that gravity couples to quantum superposition in ways beyond $\langle T_{\mu\nu}\rangle$. More recently, relativistic quantum information studies have shown that curved spacetime modifies coherence properties of quantum fields.
...
\end{equation}
When $\rho \approx \sigma[g]$, we have $\Tmn \approx 0$ and standard semiclassical gravity is recovered. When $\rho$ is a coherent state distinct from the thermal reference, $\Tmn \neq 0$ and new effects appear.

The framework, which we call \emph{Coherism}, yields the following concrete results:
\begin{enumerate}
    \item \textbf{Primary test (analog gravity):} In BEC sonic horizons, $\kappa_{\mathrm{eff}} \sim 10^{-8}$ is 62 orders of magnitude larger than the gravitational coupling, yielding $\delta\rho/\rho_0 \sim 10^{-6}$---measurable with current phase-contrast imaging.
    \item \textbf{Falsification criterion:} A null result at $\delta\rho/\rho_0 < 10^{-7}$ for coherent phonon injection would falsify the acoustic implementation.
    \item \textbf{Distinguishing test:} Coherent vs.\ thermal states of identical energy produce different $\Tmn$---a qualitative signature absent in competing frameworks.
    \item \textbf{Gravitational extensions (outlook):} The framework predicts WEP violation $\eta_{\mathrm{coh}} \sim 10^{-15}$, currently 2--3 orders below sensitivity; these are discussed in Appendix~\ref{appendix:experiments-detailed}.
\end{enumerate}
...
\emph{Scope:} We present an EFT-level variational framework with a falsifiable analog-gravity prediction; cosmology and tunneling discussions are qualitative outlook and are not used in any derivation, fit, or falsification criterion.
...
Sec.~\ref{sec:experiments} presents the primary analog gravity predictions and experimental protocol.
Sec.~\ref{sec:discussion} discusses limitations, open problems, and gravitational extensions.
...
In the controlled examples in this paper, $\sigma[g]$ is chosen as the standard geometry-adapted Hadamard/KMS state for the relevant causal diamond: Minkowski vacuum in flat space, Unruh/Hartle--Hawking-type vacua near horizons, Bunch--Davies vacuum in (quasi-)de~Sitter/FRW, and a thermal KMS state at the acoustic Hawking temperature for the BEC sonic horizon.
...
% ======================================================================
% [REMOVED FROM MAIN BODY - See Appendix for gravitational extensions]
% The following sections have been moved to outlook/future work:
% - Coherence-Dependent WEP Violation (formerly Sec V)
% - Cosmological Feedback (formerly Sec VI)  
% - Black Holes and Generalized Entropy (formerly Sec VII)
% These predictions are 2-3 orders below current experimental sensitivity.
% The primary testable prediction is the analog gravity BEC experiment.
% ======================================================================
...
\emph{Relativistic quantum information (Fuentes and others):}~\cite{FuentesReview2010}
...
The manuscript and all figures generated by the included scripts are archived at \href{https://doi.org/10.5281/zenodo.17868263}{10.5281/zenodo.17868263}~\cite{AhmannZenodoCoherism2024}.
...
The FRW sketch figure is reproduced by \texttt{python3 coherism\_frw\_simulation.py} (adds SciPy).
...
\emph{Cosmological constraints:} Current Planck and BAO data constrain dark energy equation of state variations to $|w_a| \lesssim 0.3$. If $\rho_{\mathrm{coh}}$ were significant at cosmological scales, this would require $\kappa_{\mathrm{cos}} \lesssim 10^{-3}$ in natural units. A rigorous cosmological application would require extending the coherence functional to FRW backgrounds (see Appendix~\ref{appendix:frw} for a preliminary treatment).
...
Several frameworks propose modifications to gravity based on quantum properties of matter. Table~\ref{tab:comparison} compares Coherism with the leading alternatives.
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}lccc@{}}
        \hline\hline
        Feature & Coherism & Penrose-Di\'{o}si & Stochastic \\
        \hline
        Collapse mechanism & No & Yes & No \\
        State-dependent $G_{\mu\nu}$ & Yes & Yes & No \\
        Coherent vs.\ thermal & \textbf{Yes} & No & No \\
        Analog enhancement & \textbf{Yes} & No & Maybe \\
        Semiclassical limit & Yes & Approx. & Yes \\
        \hline\hline
    \end{tabular}
    \caption{Comparison of Coherism with Penrose-Di\'{o}si gravitational collapse and stochastic gravity. The key distinguishing feature is sensitivity to coherent vs.\ thermal states of identical energy.}
    \label{tab:comparison}
\end{table}

\emph{The critical distinguishing prediction:} Coherism predicts different gravitational responses for coherent versus thermal states of \emph{identical mass and energy distribution}. Consider two samples: (A) a Bose-Einstein condensate of $N$ atoms, and (B) a thermal gas of $N$ atoms at the same temperature and density. Penrose-Di\'{o}si predicts collapse based on spatial superposition size but does not distinguish these states once the wavefunction has localized. Stochastic gravity treats both identically since $\langle T_{\mu\nu}\rangle$ is the same. Coherism uniquely predicts:
\begin{equation}
    \eta_{\mathrm{coh}} = \frac{a_{\mathrm{BEC}} - a_{\mathrm{thermal}}}{g} \sim 10^{-15},
\end{equation}
a \emph{qualitative} signature absent in competing frameworks. This coherent-vs-thermal test is the experimentum crucis for Coherism.
...
To avoid over-claiming numerics, we include a minimal \emph{toy} demonstration (not a full 1+1D dynamical Casimir simulation) of the qualitative effect described as ``coherist friction'': a coherence-dependent damping term that suppresses secular growth and produces saturation.
The script \texttt{physics/generate\_data.py} integrates a single-mode oscillator with a nonlinear damping term of the schematic form
\begin{equation}
    \ddot{\phi} + \phi + \kappa\,\phi^2\,\dot{\phi} = 0,
\end{equation}
and outputs \texttt{simulation\_data.dat} and the plot in Fig.~\ref{fig:toy-friction}. This toy is intended only as a sanity check that the feedback term can produce stable saturation in a controlled setting.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\columnwidth]{toy_coherist_friction.png}
    \caption{\textbf{Toy ``coherist friction'' demonstration.} Baseline ($\kappa=0$) versus coherence-damped evolution ($\kappa>0$) for the single-mode toy model in \texttt{physics/generate\_data.py}. The plotted quantity is the oscillator energy $E=\tfrac12(\dot{\phi}^2+\phi^2)$ as a function of time.}
    \label{fig:toy-friction}
\end{figure}
...
\end{equation}
This estimate assumes $\alpha \sim O(1)$; the true value depends on the undetermined normalization of the coherence functional. The earlier estimate of $\eta_{\mathrm{coh}} \sim 10^{-15}$ (see Introduction) assumed a different (larger) value of the effective coupling $\kappa$, illustrating the current uncertainty in predictions.
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}lccc@{}}
        ...
        \hline
        \multicolumn{4}{c}{\emph{Analog gravity (primary test)}} \\
        \hline
        BEC density & $\delta\rho/\rho_0$ & $10^{-6}$ & $10^{-7}$ \\
        Phonon spectrum & $\delta N_k/N_k$ & $10^{-5}$ & $10^{-3}$ \\
        $g^{(2)}$ correlation & $\delta g^{(2)}$ & $10^{-4}$ & $10^{-3}$ \\
        \hline\hline
    \end{tabular}
    \caption{Experimental predictions from Coherism. \emph{Top:} Gravitational tests (2--3 orders below current sensitivity). \emph{Bottom:} Analog gravity tests in BEC sonic horizons (within current reach). The analog predictions constitute the critical near-term test of the framework.}
    \label{tab:predictions}
\end{table}

\emph{Critical path:} The analog gravity predictions are within reach of current BEC experiments at MIT, JILA, Technion, and MPQ. A null result at $\delta\rho/\rho_0 < 10^{-7}$ for coherent phonon injection would falsify the acoustic implementation and cast significant doubt on the gravitational extrapolation. Conversely, observation of the predicted $10^{-6}$ density modulation---specifically for coherent but not thermal phonon injection---would provide the first direct evidence of informational backreaction on geometry.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\columnwidth]{coherism_frw_results.png}
    \caption{\textbf{Coherism FRW Simulation.} Evolution of coherence density $\rho_{\mathrm{coh}}$ and equation of state $w_{\mathrm{coh}}$ in a coupled cosmology. The coherence density grows during inflation ($w_{\mathrm{coh}} \approx -1$) and decays thereafter ($w_{\mathrm{coh}} \to 0$), leaving a small remnant density $\rho_{\mathrm{coh}}/\rho_{\mathrm{tot}} \sim 10^{-10}$ today.}
    \label{fig:frw-results}
\end{figure}
...
\paragraph{Referee-proof experimental statement (one place).}
...
\end{equation}
Coherism predicts $A \propto N_{\mathrm{phonon}}$ for \emph{coherent} injected phonons (at fixed geometry and injected energy profile), while for the \emph{thermal} control with matched injected energy the leading-order prediction is $A \approx 0$ because $\rho=\sigma[g]$ implies $S(\sigma||\sigma)=0$ and hence $\Theta_{\mu\nu}=0$.
\emph{Falsification:} for the parameter regime in Fig.~\ref{fig:bec-simulation}, a null result $A < 10^{-7}$ for coherent injection at $N_{\mathrm{phonon}}\sim 10^3$ (after accounting for standard hydrodynamic non-linearities and imaging systematics) falsifies the acoustic implementation and removes the claimed analog-gravity support for the gravitational extrapolation.
...
Figure~\ref{fig:bec-simulation} shows results from a numerical simulation of the BEC sonic horizon system using the parameters above. The simulation confirms the analytic estimate: coherent phonon injection produces a density modulation $\delta\rho/\rho_0 \approx 1.4 \times 10^{-6}$ near the horizon, while thermal phonon injection produces no signal. This is the central distinguishing prediction of the framework.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\columnwidth]{bec_sonic_horizon_results.png}
    \caption{\textbf{BEC sonic horizon simulation.} Left: Predicted density modulation $|\delta\rho/\rho_0|$ versus distance from the horizon for coherent phonon injection, shown together with the baseline (no-Coherism) null prediction. The dashed blue line marks the target magnitude $10^{-6}$; the dotted red line marks the falsification threshold $10^{-7}$. Right: Scaling of the maximum modulation $A=\max_{|r-r_H|\le L_{\mathrm{coh}}}|\delta\rho/\rho_0|$ with injected phonon number $N_{\mathrm{phonon}}$, with a simple grid-resolution check (multiple radial discretizations produce the same curve within plotting accuracy). Parameters: $^{87}$Rb BEC, $c_s = 1\,\mathrm{mm/s}$, $\rho_0 = 10^{14}\,\mathrm{cm}^{-3}$, $\xi = 0.3\,\mu\mathrm{m}$, $r_H = 50\,\mu\mathrm{m}$, $L_{\mathrm{coh}} = 10\,\mu\mathrm{m}$.}
...
% ----------------------------------------------------------------------
\section{Uniqueness of the Reference State $\sigma[g]$}
...
Consider a one-parameter family of reference states $\sigma_\epsilon = \sigma[g] + \epsilon \, \delta\sigma$ where $\delta\sigma$ is a Hadamard perturbation. The relative entropy varies as:
\begin{equation}
    S(\rho || \sigma_\epsilon) = S(\rho || \sigma) - \epsilon \, \mathrm{tr}(\rho \, \sigma^{-1} \delta\sigma) + O(\epsilon^2).
\end{equation}
The linear term vanishes when $\rho = \sigma$ (equilibrium), so near-equilibrium predictions are insensitive to the choice of $\sigma$ at first order. The informational stress tensor $\Theta_{\mu\nu}$ inherits this robustness:
\begin{equation}
    \Theta_{\mu\nu}[\sigma_\epsilon] = \Theta_{\mu\nu}[\sigma] + O(\epsilon^2, (\rho - \sigma)^2).
\end{equation}
...
All data supporting the findings of this study are available within the paper and its supplementary information. The simulation code and numerical data are available at: \url{https://github.com/davidahmann/coherism/tree/main/physics}.

\subsection*{Use of AI Tools}
Large language models (Claude/Anthropic and GPT-4/OpenAI) were used for AI-assisted copy editing, literature synthesis, and symbolic manipulation during manuscript preparation. All scientific judgment, physical interpretation, derivations, and responsibility for claims made in this work are the author's own. The author has reviewed and approved all content and confirms it reflects the original work.
