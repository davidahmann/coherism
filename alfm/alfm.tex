...
\begin{algorithm*}
\caption{NEP Query and Risk Computation}
\label{alg:nep_query}
\begin{algorithmic}[1]
\Require Context embedding $\zctx$, tenant ID $t$, threshold $\theta$, sensitivity $\alpha$
\Ensure Risk signal $\sNEP$, retrieved entries $\mathcal{R}$
\Function{NEPQuery}{$\zctx$, $t$, $\theta$, $\alpha$}
    \State $\mathcal{R} \gets \emptyset$
    \State $\mathcal{N}_{\mathrm{scope}} \gets \mathcal{N}_G \cup \mathcal{N}_{D(t)} \cup \mathcal{N}_t$ \Comment{Scoped memory}
    \For{$e \in \mathcal{N}_{\mathrm{scope}}$}
        \If{$\mathrm{sim}(\zctx, z_e) > \theta$}
            \State $\mathcal{R} \gets \mathcal{R} \cup \{e\}$
        \EndIf
    \EndFor
    \State $\sNEP^{(0)} \gets 1 - \prod_{e \in \mathcal{R}} \left(1 - \alpha \cdot \mathrm{sim}(\zctx, z_e) \cdot \frac{s_e}{5}\right)$
    \If{$|\mathcal{R}| > 1$} \Comment{Correlation correction}
        \State $\rho_{\max} \gets \max_{e_i, e_j \in \mathcal{R}} \mathrm{sim}(z_{e_i}, z_{e_j})$
        \State $\sNEP \gets \sNEP^{(0)} + \beta \cdot \rho_{\max}$
    \Else
        \State $\sNEP \gets \sNEP^{(0)}$
    \EndIf
    \State \Return $\sNEP$, $\mathcal{R}$
\EndFunction
\end{algorithmic}
\end{algorithm*}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[node distance=0.8cm, >=stealth]
        \tikzstyle{box} = [rectangle, draw, fill=blue!15, text width=1.5cm, text centered, rounded corners, minimum height=0.7cm, font=\small]
        \tikzstyle{dec} = [diamond, draw, fill=orange!15, aspect=2.5, inner sep=1pt, font=\small]
        \tikzstyle{act} = [rectangle, draw, fill=green!15, minimum width=1.0cm, text centered, rounded corners, minimum height=0.6cm, font=\small]
        \tikzstyle{arr} = [draw, ->, thick]

        % Row 1: Input centered
        \node [box] (input) {Input $\zctx$};
        
        % Row 2: Two agents
        \node [box, below left=1.0cm and 0.8cm of input] (sem) {Semantic};
        \node [box, below right=1.0cm and 0.8cm of input] (heur) {Heuristic};
        
        % Row 3: Aggregator centered, Decision on right
        \node [box, below=2.2cm of input] (agg) {Aggregator};
        \node [dec, right=0.6cm of agg] (crit) {Crit?};
        
        % Row 4: Actions spread out
        \node [act, below=1.0cm of agg, xshift=-2.4cm] (trust) {Trust};
        \node [act, below=1.0cm of agg, xshift=-0.8cm] (clar) {Clarify};
        \node [act, below=1.0cm of agg, xshift=0.8cm] (abst) {Abstain};
        \node [act, below=1.0cm of agg, xshift=2.4cm] (esc) {Escalate};
        
        % Flow arrows
        \path [arr] (input) -- (sem);
        \path [arr] (input) -- (heur);
        \path [arr] (sem) -- node[left, font=\scriptsize] {$\sNEP$} (agg);
        \path [arr] (heur) -- (crit);
        \path [arr] (crit.west) -- node[above, font=\scriptsize] {No} (agg.east);
        \path [arr] (crit.south) -- node[right, font=\scriptsize] {Yes} (esc.north);
        \path [arr] (agg) -- (trust);
        \path [arr] (agg) -- (clar);
        \path [arr] (agg) -- (abst);
        \path [arr] (agg) -- (esc);
    ...
    \caption{Consensus Engine flow. Semantic Agent provides NEP-based risk; Heuristic Agent checks rules. Critical violations bypass aggregation and escalate directly.}
...
\end{equation}
where the \emph{voting loss} $\mathcal{L}_{\mathrm{vote}}$ is the cross-entropy between predicted action $a$ and ground-truth action $a^*$:
\begin{equation}
    \mathcal{L}_{\mathrm{vote}}(a, a^*) = -\sum_{k \in \{\mathrm{trust}, \mathrm{abstain}, \mathrm{escalate}, \mathrm{clarify}\}} a^*_k \log a_k,
\end{equation}
and the \emph{calibration loss} $\mathcal{L}_{\mathrm{calib}}$ penalizes miscalibration between predicted error probability and observed error rate:
\begin{equation}
    \mathcal{L}_{\mathrm{calib}} = \sum_{b=1}^{B} \frac{|B_b|}{n} \left| \mathrm{acc}(B_b) - \mathrm{conf}(B_b) \right|,
\end{equation}
where $B_b$ is the $b$-th confidence bin, $\mathrm{acc}(B_b)$ is the empirical accuracy within the bin, and $\mathrm{conf}(B_b)$ is the mean predicted confidence. We set $\lambda_1 = 1.0$ and $\lambda_2 = 0.5$.

\begin{algorithm*}
\caption{Consensus Engine Decision}
\label{alg:consensus}
\begin{algorithmic}[1]
\Require Risk signal $\sNEP$, heuristic flags $\mathcal{H}$, context $\zctx$, confidence $c$
\Ensure Action $a \in \{\textsc{Trust}, \textsc{Abstain}, \textsc{Escalate}, \textsc{Clarify}\}$
\Function{ConsensusDecide}{$\sNEP$, $\mathcal{H}$, $\zctx$, $c$}
    \State \Comment{Phase 1: Heuristic Agent (Hard Veto Check)}
    \If{$\exists h \in \mathcal{H}$ with $h.\mathrm{critical} = \textsc{True}$}
        \State \Return \textsc{Escalate}, $h.\mathrm{reason}$ \Comment{Hard veto}
    \EndIf
    \State $v_H \gets |\{h \in \mathcal{H} : h.\mathrm{violated}\}|$ \Comment{Violation count}
    \State
    \State \Comment{Phase 2: Semantic Agent (NEP-based intuition)}
    \State $w_S \gets \mathbf{1}[c > 0.9] \cdot 2 + \mathbf{1}[c \le 0.9] \cdot 1$ \Comment{Confidence weight}
    \State $v_S \gets \sNEP \cdot w_S$
    \State
    \State \Comment{Phase 3: Vote Aggregation}
    \State $\kappa \gets \sigma(W_\kappa [v_H; v_S; \zctx] + b_\kappa)$ \Comment{Consensus score}
    \If{$v_H > 0$ \textbf{and} $\sNEP > \tau_{\mathrm{high}}$}
        \State \Return \textsc{Escalate}
    \ElsIf{$\sNEP > \tau_{\mathrm{med}}$ \textbf{or} $v_H > 0$}
        \State \Return \textsc{Abstain}
    \ElsIf{$\kappa < \tau_{\mathrm{clarify}}$}
        \State \Return \textsc{Clarify}
    \Else
...
\textbf{Cold Start Bootstrapping.}
...
\begin{algorithm*}
\caption{Bounded Adapter Update}
\label{alg:adapter}
\begin{algorithmic}[1]
\Require Correction loss $\mathcal{L}_{\mathrm{corr}}$, adapter $\Delta_t$, learning rate $\eta$, clip bound $\gamma$, max norm $\delta_{\max}$
\Ensure Updated adapter $\Delta_t'$
\Function{AdapterUpdate}{$\mathcal{L}_{\mathrm{corr}}$, $\Delta_t$, $\eta$, $\gamma$, $\delta_{\max}$}
    \State $g \gets \nabla_{\Delta_t} \mathcal{L}_{\mathrm{corr}}$ \Comment{Compute gradient}
    \State
    \State \Comment{Gradient clipping}
    \If{$\|g\|_F > \gamma$}
        \State $g \gets g \cdot (\gamma / \|g\|_F)$
    \EndIf
    \State
    \State \Comment{Apply update}
    \State $\Delta_t' \gets \Delta_t - \eta \cdot g$
    \State
    \State \Comment{Norm constraint (projection to feasible set)}
    \If{$\|\Delta_t'\|_F > \delta_{\max}$}
        \State $\Delta_t' \gets \Delta_t' \cdot (\delta_{\max} / \|\Delta_t'\|_F)$
    \EndIf
    \State
...
\subsubsection{RAG Baseline Implementation}
To ensure fair comparison, the RAG baseline implements failure memory via retrieval augmentation:
\begin{itemize}
    \item \textbf{Index:} Same HNSW configuration as NEP (Section~\ref{app:implementation}).
    \item \textbf{Entry Format:} Each failure is stored as natural language: ``\texttt{[FAILURE] Context: <ctx>. Error: <err>. Correction: <corr>}''.
    \item \textbf{Retrieval:} Top-$k$ ($k=5$) failures prepended to prompt as few-shot examples.
    \item \textbf{No Risk Signal:} RAG provides information but not calibrated risk; the backbone decides confidence implicitly.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{ALFM$_{\text{--NEP}}$:} Consensus Engine only, with $\sNEP = 0$ (no failure memory). Tests whether heuristic rules alone provide safety.
    \item \textbf{ALFM$_{\text{--CE}}$:} NEP only, with fixed threshold-based action ($\sNEP > 0.5 \Rightarrow$ Abstain). Tests whether learned arbitration adds value.
    \item \textbf{ALFM$_{\text{--Adapt}}$:} No adapters; projection frozen after initial training. Tests continual learning contribution.
    \item \textbf{ALFM$_{\text{Global}}$:} Global NEP only ($\N_T = \emptyset$). Tests tenant-specific memory value.
\end{enumerate}
...
\begin{table}[htbp]
    \centering
    \caption{Projected latency breakdown by hardware tier and NEP size. All values in milliseconds. Backbone latency (not shown) dominates total inference time ($\approx 200$--$2000$ms for API-based models).}
    \label{tab:latency}
    \begin{tabular}{l c c c c c}
        \toprule
        \textbf{Hardware} & \textbf{$|\N|$} & $\tau_{\mathrm{proj}}$ & $\tau_{\mathrm{NEP}}$ & $\tau_{\mathrm{CE}}$ & \textbf{Total} \\
        \midrule
...
\begin{lstlisting}[language=Python, caption={ALFM Wrapper API}, label={lst:api}]
from alfm import ALFMWrapper, NEPMemory, ConsensusEngine
from alfm.adapters import TenantAdapter

# Initialize with any backbone (OpenAI, Anthropic, local)
backbone = OpenAIClient(model="gpt-4")
alfm = ALFMWrapper(
    backbone=backbone,
    nep=NEPMemory(index_path="./nep_index"),
    consensus=ConsensusEngine.from_pretrained("alfm-ce-v1"),
    tenant_id="tenant_acme_corp"
)

# Standard inference with ALFM safety layer
result = alfm.query(
    prompt="Summarize the Q3 financial report",
    context={"document": doc_text}
)

# Result includes action recommendation
if result.action == "trust":
    print(result.output)
elif result.action == "abstain":
    print(f"Abstaining: {result.reason}")
    print(f"Risk signal: {result.risk_score:.2f}")
elif result.action == "escalate":
    escalate_to_human(result)

# Record feedback for NEP learning
alfm.record_failure(
    context=result.context_embedding,
    severity=4,
    correction="Revenue figure was hallucinated",
...
This appendix provides concrete examples for each of the five canonical failure types used in seed NEP initialization (Section~\ref{sec:validation}). These examples are domain-agnostic templates; production deployments should augment with domain-specific failure patterns.

\subsection{Hallucination}
Factual claims that contradict verifiable knowledge.

\textbf{Example 1 (Healthcare):} ``The patient's A1C level of 5.2\% indicates poorly controlled diabetes.'' (Incorrect: 5.2\% is within normal range.)

\textbf{Example 2 (Finance):} ``Apple Inc. reported Q3 2024 revenue of \$127 billion, a 15\% increase YoY.'' (Fabricated specific figure without source verification.)

\textbf{Example 3 (Legal):} ``Under the GDPR Article 45, data transfers to the US are unconditionally permitted.'' (Incorrect: Schrems II invalidated Privacy Shield.)

\subsection{Policy Violation}
Outputs that violate explicit domain constraints or organizational rules.

\textbf{Example 1 (Healthcare):} Recommending a specific dosage without physician oversight when policy requires ``consult your healthcare provider.''

\textbf{Example 2 (Finance):} Providing specific investment advice (``Buy AAPL stock'') when policy restricts to educational content only.

\textbf{Example 3 (Legal):} Offering legal interpretation as definitive advice rather than general information with disclaimer.

\subsection{Reasoning Error}
Logical inconsistencies, invalid inferences, or computational mistakes.

\textbf{Example 1:} ``If revenue increased 20\% and costs increased 30\%, then profit margin improved.'' (Invalid inference: margin likely decreased.)

\textbf{Example 2:} ``The contract expires in 90 days from March 1, 2024, which is May 30, 2024.'' (Incorrect: should be May 30 or May 31 depending on counting method, but calculation shown is wrong.)

\textbf{Example 3:} ``Since A implies B, and B is true, therefore A must be true.'' (Affirming the consequent fallacy.)

\subsection{Instruction Drift}
Outputs that ignore, misinterpret, or only partially address user intent.

\textbf{Example 1:} User asks for ``a bulleted summary'' but receives a narrative paragraph.

\textbf{Example 2:} User requests ``risks only'' but output includes both risks and opportunities.

\textbf{Example 3:} User specifies ``in formal tone'' but output uses casual language with contractions.

\subsection{Unsafe Content}
Outputs that could cause harm, violate safety guidelines, or contain inappropriate material.

\textbf{Example 1:} Providing detailed instructions for activities that could be dangerous without appropriate safety warnings.

\textbf{Example 2:} Generating content that could be used for social engineering or phishing.

\textbf{Example 3:} Reproducing copyrighted material verbatim when asked to ``summarize.''

\subsection{Severity Assignment Guidelines}
Each failure type is assigned severity $s \in \{1, 2, 3, 4, 5\}$ based on potential impact:
\begin{itemize}
    \item $s=1$: Minor stylistic deviation, easily corrected
    \item $s=2$: Noticeable error, low downstream impact
    \item $s=3$: Material error requiring correction before use
    \item $s=4$: Significant error with potential for harm or liability
    \item $s=5$: Critical failure requiring immediate escalation
\end{itemize}
Hallucinations in high-stakes domains (medical, legal, financial) default to $s \ge 4$. Policy violations inherit severity from the violated policy's criticality tier.
