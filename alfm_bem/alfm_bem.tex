...
% JMLR style - use nohyperref so we can configure hyperref ourselves
\usepackage[nohyperref]{jmlr2e}

% Load hyperref with clickable colored links
\usepackage[colorlinks=true,linkcolor=blue,citecolor=blue,urlcolor=blue,breaklinks=true]{hyperref}
\urlstyle{same}

% Make \url create clickable hyperlinks - must be done after document begins
% to override the \providecommand in the .bbl file
\AtBeginDocument{%
...
\begin{table}[t]
\centering
\caption{RAG vs. BEM: Conceptual comparison. RAG retrieves information; BEM retrieves experiences with outcomes.}
\label{tab:rag_bem}
\begin{tabular}{lcc}
\toprule
\textbf{Property} & \textbf{RAG} & \textbf{BEM} \\
\midrule
Stores & Documents/facts & Experiences (context + outcome) \\
Learns from deployment & \xmark & \cmark \\
Knows what failed & \xmark & \cmark \\
Knows what succeeded & \xmark & \cmark \\
OOD detection & \xmark & \cmark~(coverage signal) \\
Outcome-aware retrieval & \xmark & \cmark \\
Requires human curation & \cmark & \xmark~(learns from outcomes) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{ALFM-BEM Core Contract vs. Optional Modules.}
\textbf{Core (minimum system):} frozen backbone $\rightarrow$ projection $\phi$ $\rightarrow$ \bem{} (risk/success/coverage) $\rightarrow$ Consensus Engine $\rightarrow$ \{Trust, Abstain, Escalate, Query\} $\rightarrow$ outcome feedback $\rightarrow$ \bem{} update.
\textbf{Optional modules (engineering extensions):} bounded adapters with replay, multi-tenant scoping (global/domain/tenant), anti-poisoning validation, approximate indexing, and memory hygiene (vacuum/pruning).
\end{minipage}}
\end{center}
...
\textbf{Note.} Experience retrieval is not a correctness oracle. It provides outcome-conditioned control evidence for the Consensus Engine; correctness still depends on the backbone and downstream verification.

\textbf{Definition 3.4 (Coverage Signal).} The coverage signal measures how well $z$ is represented in the experience distribution---an emergent property, not a separate detector. We use a KDE-style density over cosine similarities with a data-dependent calibration constant:
\begin{equation}
...
\end{equation}
where $h$ is the KDE bandwidth (we use $h = 0.3$, selected via cross-validation) and $\kappa$ is chosen per deployment/memory so that the 90th percentile of in-distribution self-density maps to 1.0 (a robust normalization). This formulation captures local density in experience space. For clustered patterns, KDE coverage achieves near-perfect OOD detection (AUC $\approx$ 1.0) because novel clusters have no nearby experiences. Low coverage indicates out-of-distribution operation.
...
\textbf{Latency Overhead.} The total latency overhead is $\tau_{\mathrm{ALFM}} = \tau_{\mathrm{proj}} + \tau_{\mathrm{BEM}} + \tau_{\mathrm{CE}}$. The operations are sequential. Projection involves a matrix multiplication of size $d_z \times d_h$. BEM query with approximate nearest-neighbor indexing takes $O(\log |\N|)$. Consensus Engine inference is a small MLP/Logic block. The values in Table~\ref{tab:latency} are illustrative estimates; exact latencies depend on indexing choice, batching, and hardware.
...
\textbf{Proposition 4.1 (BEM Query Complexity).} With approximate nearest neighbor indexing (e.g., HNSW or KD-Trees), BEM query complexity is $O(\log |\mathcal{E}|)$ where $|\mathcal{E}|$ is the number of stored experiences.

\textit{Proof sketch.} Hierarchical index structures construct a navigable graph or tree with $O(\log |\mathcal{E}|)$ depth. Each query traverses at most $O(\log |\mathcal{E}|)$ nodes, yielding logarithmic complexity. The risk/success/coverage signals require constant-time aggregation over the $k$ nearest neighbors (typically $k \leq 100$). \qed

\subsection{Tenant Isolation}

\textbf{Proposition 4.2 (Tenant Isolation).} Experiences are scope-partitioned and access-controlled: tenant $t$ can only read $\mathcal{E}_{\text{global}} \cup \mathcal{E}_{D(t)} \cup \mathcal{E}_t$. Cross-tenant leakage requires an access-control failure.
...
\textbf{Proposition 4.3 (Coverage tracks density).} As $|\mathcal{E}| \to \infty$ with experiences drawn i.i.d. from the deployment distribution $P$, the un-clipped KDE term converges to a quantity proportional to the local density $p(z)$ for $z \sim P$. For $z$ in regions of low $p(z)$ (including novel clustered modes), the coverage signal decreases correspondingly.
...
\end{equation}
For $z \sim P$: as $|\mathcal{E}| \to \infty$, the empirical KDE converges to the expectation under $P$ by the law of large numbers, yielding a value proportional to local density $p(z)$.

For regions where $p(z)$ is low (including $z$ from a shifted clustered distribution), cosine similarities $\text{sim}(z, z_e)$ are typically small, so the Gaussian kernel values decay rapidly as similarity decreases, yielding low coverage. \qed
...
\begin{itemize}
    \item 500 failures clustered around 10 failure modes ($\sigma = 0.05$), with an overlap fraction of 0.3 near success modes (hard negatives)
    \item 500 successes clustered around 5 success modes ($\sigma = 0.05$), with an overlap fraction of 0.3 near failure modes
...
\begin{itemize}
    ...
    \item \textbf{Adapter Norm}: Parameter norm drift under training
\end{itemize}

\subsection{Results}

\textbf{BEM Retrieval (Table~\ref{tab:retrieval}).} BEM achieves moderate failure retrieval (F1 $\approx$ 0.59) comparable to baselines, but uniquely provides success pattern retrieval (F1 $\approx$ 0.72) that neither RAG nor NEP can offer. This bidirectional capability is BEM's key differentiator.

\begin{table}[t]
\centering
\caption{BEM retrieval performance on synthetic deployment data (mean $\pm$ std over 5 seeds).}
\label{tab:retrieval}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{Failure} & \textbf{Success} \\
\midrule
Precision & 0.57 $\pm$ 0.03 & 0.73 $\pm$ 0.02 \\
Recall & 0.61 $\pm$ 0.04 & 0.71 $\pm$ 0.00 \\
F1 & 0.59 $\pm$ 0.03 & 0.72 $\pm$ 0.01 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{OOD Detection (Figure~\ref{fig:ood}).} For detecting novel clustered patterns (identifying novel failure modes not seen in training), the coverage signal achieves AUC $\approx$ 1.0. In the canonical setup, mean coverage for in-distribution clustered failures is 0.47; for OOD samples, 0.10. This validates that a useful OOD signal emerges from BEM without a separate detector.

\textbf{Adapter Stability (Figure~\ref{fig:drift}).} With norm projection ($c_\theta=2.0$), adapter parameters remain bounded (final $\|\theta\|_F \approx 2.0$). Removing the constraint increases the final norm to $\|\theta\|_F \approx 8.3$ under the same update schedule. The norm projection constraint provably keeps parameters within the stable region.

\textbf{Consensus Engine Behavior (Table~\ref{tab:consensus}).} The Consensus Engine implements threshold-based arbitration: low coverage triggers Query, high risk triggers Abstain/Escalate, and high success evidence yields Trust.

\begin{table}[t]
\centering
\caption{Consensus Engine decision rules (semantic agent).}
\label{tab:consensus}
\begin{tabular}{lcc}
\toprule
\textbf{Condition} & \textbf{Signal} & \textbf{Action} \\
\midrule
High risk & $r > 0.6$ & Abstain / Escalate \\
Low coverage (OOD) & $c < 0.3$ & Query \\
Strong success evidence & $s > 0.7$ & Trust \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Adapter parameter norm. Bounded training (blue) vs. unbounded (red).}
\label{fig:drift}
\end{minipage}
\end{figure}
...
\subsection{Real-World Embedding Validation}
\label{sec:real_embeddings}

To validate BEM's core mechanism beyond synthetic data, we conducted an experiment using real-world text embeddings from the 20Newsgroups dataset~\citep{Lang1995} and the \texttt{all-MiniLM-L6-v2} SentenceTransformer model~\citep{Reimers2019}.

\textbf{Setup.} We mapped semantic topics to key deployment concepts:
\begin{itemize}
    \item \textbf{In-Distribution (Science)}: \texttt{sci.space}, \texttt{sci.med}, \texttt{sci.electronics}.
    \item \textbf{Out-of-Distribution (Sports)}: \texttt{rec.sport.hockey}, \texttt{rec.sport.baseball}.
    \item \textbf{Failure Modes}: We designated specific technical sub-topics (e.g., ``orbit'', ``circuit'') within the Science domain as failure modes, simulating a model with specific knowledge gaps.
\end{itemize}

We trained a projection layer ($\phi: \mathbb{R}^{384} \to \mathbb{R}^{64}$) using contrastive loss to map failures closer to other failures than to successes.

\textbf{Results.} Table~\ref{tab:real_emb} demonstrates that BEM's projection layer significantly improves failure retrieval precision compared to raw embeddings. Raw embeddings, despite being semantically rich, do not naturally cluster by ``correctness''---a failure on ``orbit'' is semantically close to a success on ``planet''. The projection layer successfully re-organizes the space to prioritize outcome-relevant similarity.

\begin{table}[ht]
\centering
\caption{Retrieval performance on Real-World Text Data (20Newsgroups). Validates that BEM's projection layer is essential and effective for real embeddings.}
\label{tab:real_emb}
\begin{tabular}{lcc}
\toprule
\textbf{Embedding Type} & \textbf{Failure Retrieval (P@5)} & \textbf{Improvement} \\
\midrule
Raw SBERT (384D) & 0.73 & -- \\
BEM Projected (64D) & \textbf{0.95} & \textbf{+30\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Results (Table~\ref{tab:ablation}).} On our synthetic data with overlapping distributions, RAG achieves the highest failure F1 because it retrieves similar items regardless of outcome---but this comes at the cost of \emph{no outcome awareness}. The key differentiator is success retrieval: only BEM can answer ``how did we handle this successfully before?''

\begin{table}[t]
\centering
\caption{Ablation: BEM vs.\ simpler baselines (mean $\pm$ std over 5 seeds).}
\label{tab:ablation}
\begin{tabular}{lcccc}
\toprule
\textbf{System} & \textbf{Fail F1} & \textbf{Success F1} & \textbf{OOD AUC (clust)} & \textbf{OOD AUC (dist)} \\
\midrule
Plain RAG & 0.74 \small{$\pm$ 0.02} & 0.00 \small{$\pm$ 0.00} & 0.99 \small{$\pm$ 0.01} & 0.99 \small{$\pm$ 0.01} \\
NEP & 0.57 \small{$\pm$ 0.02} & 0.00 \small{$\pm$ 0.00} & 0.99 \small{$\pm$ 0.01} & 0.99 \small{$\pm$ 0.00} \\
BEM (ours) & 0.59 \small{$\pm$ 0.03} & \textbf{0.72} \small{$\pm$ 0.01} & 0.88 \small{$\pm$ 0.12} & 0.95 \small{$\pm$ 0.01} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{KDE coverage}: In the canonical clustered-OOD setting (Figure~\ref{fig:ood}), KDE coverage achieves AUC $\approx$ 1.0. In the overlapping ablation setup, OOD AUC exhibits higher variance (Table~\ref{tab:ablation}), consistent with density overlap and calibration sensitivity.
...
\textbf{Query Action Effectiveness.} To validate the active learning loop, we simulated a decision boundary with an ambiguity zone where model confidence drops to 0.5. We compared three policies: (1) Trust Model, (2) Passive Abstain (threshold 0.7), and (3) Active Query (query top-15\% most uncertain).

\begin{itemize}
    \item \textbf{Baseline Accuracy}: 93.4\%
    \item \textbf{Passive Abstain}: 98.0\% (10.9\% abstention rate)
    \item \textbf{Active Query}: \textbf{99.2\%} (15\% query budget)
\end{itemize}

The Query action bridges the gap between passive safety and high performance, delivering a \textbf{6.2\% improvement} in accuracy over the baseline by actively resolving ambiguity in the decision boundary region.
...
\begin{figure}[t]
    ...
    \caption{Learning curve for healthcare claims. The rejection rate on submitted claims drops from 11.6\% (baseline) to 1.2\% (final window) as BEM learns the latent payer rules from binary feedback. The abstain (human review) rate stabilizes at $\approx 16\%$.}
    \label{fig:healthcare_learning}
\end{figure}

\textbf{Rejection Reduction.} In the canonical run (seed 42), rejection-on-submitted dropped from 11.6\% to 1.2\%---a $\approx$90\% reduction. The system learned latent rules from outcomes alone and shifted borderline cases into abstain (human review), trading throughput for lower denial risk.
...
\textbf{Bounded interventions (whack-a-mole).} Outcome-conditioned updates can shift behavior into a different failure basin: suppressing one failure mode may expose another. ALFM-BEM mitigates this by bounding interventions (adapter drift constraints), mediating behavior changes through the Consensus Engine, and preferring Query/Escalate when uncertainty is high rather than aggressive updates.

\subsubsection{Outcome Signal Reliability}

BEM assumes outcome signals $o \in [-1, 1]$ are available and meaningful. This assumption has several failure modes:

\textbf{Delayed feedback.} In many domains (e.g., healthcare claims, loan approvals), outcomes arrive days or weeks after inference. BEM handles this via asynchronous experience ingestion---experiences are timestamped at inference but outcomes are attached when available. The coverage signal remains valid immediately; risk/success signals update as outcomes arrive.

\textbf{Noisy labels.} User feedback may be incorrect (e.g., marking a correct response as wrong). We mitigate this via: (1) confirmation thresholds requiring multiple independent confirmations for high-severity labels (Section~\ref{sec:antipoisoning}); (2) confidence decay that down-weights unconfirmed experiences over time; (3) cross-tenant consensus for global BEM updates.

\textbf{Adversarial poisoning.} Malicious users could submit false outcomes to corrupt BEM. Section~\ref{sec:antipoisoning} details our multi-layered defense: anomaly detection on submission velocity, quarantine for suspicious patterns, and administrative approval for high-impact updates.

\textbf{Outcome ambiguity.} Some interactions have no clear success/failure signal (e.g., open-ended conversations). BEM supports partial outcomes via the continuous spectrum; interactions without clear outcomes can be stored with $o \approx 0$ (neutral) and contribute to coverage without affecting risk/success signals.

\subsubsection{Coverage Signal Failure Modes}

The coverage signal detects OOD inputs as an emergent property of experience density. Known failure modes include:

\textbf{False confidence (high coverage, wrong answer).} Coverage measures \emph{familiarity}, not \emph{correctness}. An input similar to many past experiences will have high coverage even if the backbone produces a wrong answer. Mitigation: the success signal provides outcome-aware confidence. High coverage with low success signal triggers the Query action.

\textbf{Projection collapse.} If the contrastive projection layer maps semantically different inputs to similar embeddings, coverage becomes unreliable. We mitigate this via hard negative mining during projection training (Section~\ref{sec:arch}): experiences with divergent outcomes must map to different regions.

\textbf{Distribution shift.} Gradual distribution shift can cause coverage to remain high even as the deployment distribution diverges from training. We recommend periodic coverage calibration: if mean coverage on a held-out sample drops below a threshold, the projection layer should be retrained.

\textbf{Uniform OOD.} Coverage via KDE excels at detecting \emph{clustered} novel patterns (AUC $\approx$ 1.0) but has moderate performance on uniformly distributed OOD inputs (AUC $\approx$ 0.96). For domains where uniform OOD is expected, we recommend combining coverage with energy-based OOD detection on the backbone.

\subsubsection{BEM Growth and Lifecycle}

Memory grows with deployment, requiring lifecycle management:

\textbf{Storage economics.} Each experience requires $\approx$256 bytes (64D float32 embedding + metadata). At 1M experiences, BEM requires $\approx$256MB storage. HNSW indexing adds $\approx$2$\times$ overhead. For cost-sensitive deployments, we recommend tiered storage: hot (recent, high-weight) experiences in memory, cold experiences on disk with lazy loading.

\textbf{Memory pruning and bounds.} Each tenant operates under a fixed memory budget (e.g., max $N$ experiences or a storage quota). At saturation, the system deterministically retains the most useful experiences (high confirmation count, high severity, and recent) and prunes low-utility entries first. A time-to-live (TTL) policy can drop old, low-confirmation experiences while retaining strongly confirmed or high-severity failures indefinitely. Pruning runs during maintenance windows.

\textbf{Multi-tenant cost allocation.} Per-tenant BEM partitions enable usage-based billing. Each tenant's storage footprint is tracked; tenants exceeding quota receive Query actions prompting them to approve archival of old experiences.

\textbf{Cold start.} New deployments have empty BEM. We provide cold-start strategies: (1) synthetic seeding from the failure taxonomy (Appendix~\ref{app:taxonomy}); (2) transfer from similar tenants (with consent); (3) conservative Consensus Engine thresholds that default to Query until coverage builds.
...
\begin{itemize}
    ...
    \item Healthcare case study uses a simplified simulator; production payer rules are more complex
    \item Projection layer requires retraining when backbone changes
    \item Query action effectiveness depends on user willingness to provide clarification
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Privacy concerns}: Experience memory stores deployment contexts. Mitigation: tenant isolation via scope-partitioning and access control; periodic vacuuming removes old experiences.
...
\State $\mathcal{R} \leftarrow \textsc{TopK}(\mathcal{R}, k\text{ by }\text{sim}(z, z_e))$
\State $s_{\text{risk}} \leftarrow 0$
\For{$e \in \mathcal{R}$}
    \State $w \leftarrow \alpha \cdot \text{sim}(z, z_e) \cdot \frac{\text{sev}(e)}{5} \cdot \text{decay}(e) \cdot \text{conf}(e)$
...
We generate successes from clustered success modes, with an overlap fraction sampled near failure modes to create semantically similar but outcome-divergent neighbors.

OOD samples are generated as a shifted tight cluster (a novel centroid) to test detection of novel clustered modes.
...
\section{Projection Layer Details}
\label{app:projection}

The projection layer $\phi: \mathbb{R}^{d_{backbone}} \to \mathbb{R}^{d_{BEM}}$ is a standard MLP with one hidden layer:
\begin{equation}
    \phi(x) = \text{LayerNorm}(W_2(\text{ReLU}(W_1(x) + b_1)) + b_2)
\end{equation}

\textbf{Training Procedure}:
\begin{itemize}
    \item \textbf{Loss function}: Triplet Margin Loss / Contrastive Loss.
    \item \textbf{Margin}: $\alpha = 1.0$.
    \item \textbf{Optimizer}: Adam, $lr=10^{-3}$.
    \item \textbf{Negative Mining}: We construct batches with (Anchor, Positive, Negative) triplets where:
    \begin{itemize}
        \item \textit{Anchor}: A failure experience.
        \item \textit{Positive}: Another failure experience (simulating same mode).
        \item \textit{Negative}: A success experience (simulating distinct mode).
    \end{itemize}
\end{itemize}

\textbf{Cold Start}: Initially, $\phi$ is initialized as a random projection. It is trained offline on a "seed set" of generic failure/success pairs from public datasets (e.g., toxicity datasets for safety failures) before specific deployment data is available.
